...
class UserRepository:
    ...
    async def find_by_id(self, user_id: uuid.UUID) -> Optional[User]:
        ...
        result = await self.session.execute(bd_query, {"id": str(user_id)})
    ...
    async def find_by_email(self, email: str) -> Optional[User]:
        ...
        result = await self.session.execute(bd_query, {"email": email})
    ...
    async def find_all(self) -> List[User]:
        ...
        users = []
        for row in rows:
            users.append(User(id=row[0], email=row[1], name=row[2], created_at=row[3]))
        return users
...
class OrderRepository:
    ...
    async def find_by_id(self, order_id: uuid.UUID) -> Optional[Order]:
        ...
        items_result = await self.session.execute(
            bd_items_query, {"order_id": order_id}
        )
        ...
        history_result = await self.session.execute(
            bd_history_query, {"order_id": order_id}
        )
